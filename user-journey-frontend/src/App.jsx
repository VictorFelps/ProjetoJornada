...
// Paleta de cores para os canais
...
function App() {
  ...
  const [channelColorMap, setChannelColorMap] = useState(new Map())
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Criar mapeamento de cores para canais
        const allChannels = new Set()
        data.journeys.forEach(journey => {
          journey.journey.forEach(channel => allChannels.add(channel))
        })

        const colorMap = new Map()
        Array.from(allChannels).forEach((channel, index) => {
          colorMap.set(channel, CHANNEL_COLORS[index % CHANNEL_COLORS.length])
        })
        setChannelColorMap(colorMap)
  ...
  const getChannelColor = (channel) => {
    return channelColorMap.get(channel) || 'bg-gray-100 text-gray-800 border-gray-200'
  }
  ...
  const formatJourney = (journey) => {
    return journey.map((channel, index) => (
      <span key={index} className="inline-flex items-center">
        <Badge 
          variant="outline" 
          className={`mr-1 ${getChannelColor(channel)} font-medium`}
        >
          {channel}
        </Badge>
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="container mx-auto p-6">
        ...
        {/* Channel Legend */}
        {channelColorMap.size > 0 && (
          <Card className="mb-6">
            <CardHeader>
              <CardTitle className="text-lg">Legenda de Canais</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="flex flex-wrap gap-2">
                {Array.from(channelColorMap.entries()).map(([channel, colorClass]) => (
                  <Badge 
                    key={channel}
                    variant="outline" 
                  ...
                  {filteredJourneys.slice(0, 20).map((journey) => (
                    <TableRow key={journey.sessionId}>
                      ...
                      <TableCell>
                        <Badge 
                          variant="outline"
                          className={`${getChannelColor(journey.firstTouchpoint.channel)} font-medium`}
                        >
                      ...
                      <TableCell>
                        <Badge 
                          variant="outline"
                          className={`${getChannelColor(journey.lastTouchpoint.channel)} font-medium`}
                        >
